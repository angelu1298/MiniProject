...
public class FireFightView extends JPanel {
	ImageIcon img;
	...
	FireFightView(ExecGame frame) {
		monster = new Monster().FireEasyMonster(); // 몬스터 생성
		ball = StageMouseListener.ball;// 볼선택 있어야 함
		String[] monname = {"파이리", "식스테일","가디","포니타","피카츄","불꽃숭이","브케인","아차모"};
		ImageIcon[] charicon = {new ImageIcon("image/001.png"),new ImageIcon("image/002.png"),new ImageIcon("image/003.png"),new ImageIcon("image/004.png"),
				new ImageIcon("image/005.png"),new ImageIcon("image/006.png"),new ImageIcon("image/007.png"),new ImageIcon("image/008.png")
		};
		for (int i = 0; i<8 ; i++){
			if(FireFieldView.flag == i)
		...
		img = new ImageIcon("image/firefight.png");
		...
		attbutton.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent arg0) {
				...
				if (num<=0) { //몬스터볼 모두 소진 시..... 전투종료
					...
					monster = new Monster().FireEasyMonster();
					monsterhp.setText("체력          " + (int) monster.getMhp() + " / " + (int) Mhp);
					frame.changePanel(new FireFieldView(frame));
					attcnt = 0;
			...
			public void mouseReleased(MouseEvent arg0) {
				...
				if (num<=0) { //몬스터볼 모두 소진 시..... 전투종료
					...
					if (n > monster.getMrate()*(1+ball.getBallGetrate() )* 100) {	// 여기 소스 언니한테 물어보기
						...
						FireFieldView.flag++;
						if (FireFieldView.finalflag <= FireFieldView.flag)
							FireFieldView.finalflag = FireFieldView.flag;
