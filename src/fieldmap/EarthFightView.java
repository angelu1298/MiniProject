...
public class EarthFightView extends JPanel {
	ImageIcon img;
	...
	EarthFightView(ExecGame frame) {
		monster = new Monster().EarthEasyMonster(); // 몬스터 생성
		ball = StageMouseListener.ball;
		String[] monname = { "구구", "꼬렛", "디그다", "나옹", "알통몬", "꼬마돌", "고오스", "이브이" };
		ImageIcon[] charicon = { new ImageIcon("image/025.png"), new ImageIcon("image/026.png"),
				new ImageIcon("image/027.png"), new ImageIcon("image/028.png"), new ImageIcon("image/029.png"),
				new ImageIcon("image/030.png"), new ImageIcon("image/031.png"), new ImageIcon("image/032.png")
		};
		for (int i = 0; i < 8; i++) {
			if (EarthFieldView.flag == i) {
		...
		img = new ImageIcon("image/earthfight.png");
		...
		JLabel level = new JLabel("Lv . " + ExecGame.player.getLevel().getLev()); // 플레이어의
																					// 현재
																					// 레벨
		...
		attbutton.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent arg0) {
				...
				if (num<=0) { //몬스터볼 모두 소진 시..... 전투종료
					...
					monster = new Monster().EarthEasyMonster();
					monsterhp.setText("체력          " + (int) monster.getMhp() + " / " + (int) Mhp);
					frame.changePanel(new EarthFieldView(frame));
					attcnt = 0;
			...
			public void mouseReleased(MouseEvent arg0) {
				...
				if (num<=0) { //몬스터볼 모두 소진 시..... 전투종료
					...
					if (n > monster.getMrate()*(1+ball.getBallGetrate() )* 100) {	// 여기 소스 언니한테 물어보기
						...
						EarthFieldView.flag++;
						if (EarthFieldView.finalflag <= EarthFieldView.flag)
							EarthFieldView.finalflag = EarthFieldView.flag;
